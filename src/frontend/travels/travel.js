...
function createAndAppendTravelCard(destination, location, imgUrl, year, parentId) {
...
function listAllTheTravelDestination(travelDestinations) {
  ...
  if (travelDestinations && Array.isArray(travelDestinations)) {
    travelDestinations.map((travel) => {
      createAndAppendTravelCard(travel.destination, travel.location, 'https://picsum.photos/200', travel.year, 'list');
    });
  }
  return;
}
